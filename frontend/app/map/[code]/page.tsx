...
/* ---------------------------
   ...
   Keep the same system prompt text you used previously.
---------------------------- */
const SYSTEM_PROMPT = `
...
export default function MapPage() {
  ...
  // IMPORTANT FIX: return LatLon[] (lat, lon) not {lat, lng}
  const toJsonCoords = (latlngs: any[]): LatLon[] =>
    ...
    try {
      ...
      if (mergedText) {
        try {
          ...
          // <-- IMPORTANT CHANGE: include system_prompt in the body
          const promptResp = await fetch(promptUrl, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ prompt: mergedText, system_prompt: SYSTEM_PROMPT }),
          });
